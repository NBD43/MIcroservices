...
public class UserServiceImpl implements UserService {
	...
	public Response onRegister(UserDTO userDto) {
		...
		try {
			...
			mailModel.setBody(url + "/valid");
			mailModel.setTo(userDto.getEmailId());
	...
	public Response forgetPassword(EmailDTO emailDto) throws UserException, UnsupportedEncodingException {
		...
		try {
			 ...
			 * rabbitmq.send(mailmodel);
			 */
			url = Utility.getUrl(id);
			System.out.println(url);

			mailModel.setBody(url + "/valid");
			mailModel.setTo(emailDto.getEmailId());
			// rabbitmq.sendMessageToQueue(mailModel);
			rabbitMqSenderImpl.sendMessageToQueue(mailModel);
			rabbitMqSenderImpl.send(mailModel);}
		catch (IllegalArgumentException e) {
	...
	@Override
	public long findUser(String emailId) {
		Optional<User> user= userRepo.findByEmailId(emailId);
		if(!user.isPresent()) {
			throw new UserException(401,"user is not present");
		}
